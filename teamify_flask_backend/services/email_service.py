...
def _mail_config() -> dict[str, str]:
    if has_app_context():
        ...
        return {
            ...
            "from_address": (cfg.get("MAIL_FROM_ADDRESS") or "").strip(),
    ...
    return {
        ...
        "from_address": (os.getenv("MAIL_FROM_ADDRESS") or "").strip(),
...
def is_mail_configured() -> bool:
    cfg = _mail_config()
    return bool(cfg["api_key"] and cfg["from_address"] and is_valid_email(cfg["from_address"]))
...
def send_email(
    *,
    ...
    if not cfg["api_key"] or not cfg["from_address"]:
