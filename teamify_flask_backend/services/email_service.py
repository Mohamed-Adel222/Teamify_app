...
def send_otp_email(user, otp: str) -> dict[str, Any] | None:
...
def send_connection_request_email(addressee, requester_name: str) -> dict[str, Any] | None:
    """Connection request email (respects user preferences)."""
    if not getattr(addressee, "email", None):
        return None
    if not user_email_pref_enabled(addressee):
        return None
    greeting = html.escape(
        (addressee.full_name or addressee.display_name or "there").strip() or "there"
    )
    ...
    return send_email(
        ...
        html_body=(
            f"<p>Hi {greeting},</p>"
            f"<p><strong>{requester}</strong> sent you a connection request on Teamify.</p>"
            "<p>Open their profile from Search to accept.</p>"
        ),
    )
